...
public class Server extends Broker {
	...
	class ActionsForPublishers extends Thread {
		...
		public void run() {
			...
			System.out.println("New publisher data created with USERNAME: " + publisherData.getConnectionMetadata().getUsername() +" IP: " + publisherData.getConnectionMetadata().getIp() + " PORT: " +publisherData.getConnectionMetadata().getPort());
