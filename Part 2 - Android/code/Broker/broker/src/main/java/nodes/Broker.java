...
public class Broker extends Node {
    ...
    public void acceptConnection(PublisherData newdata ) throws IOException, ClassNotFoundException, TikaException, SAXException {
        ...
        synchronized (publishers) {
         ...
         // get file data from publisher
        getCatalogFromPublisher(newdata);

        ConnectionMetadata metadata =newdata.getConnectionMetadata();

        while (true) {
            System.out.println("\nWaiting for a command from a publisher ");

            Message m = super.receiveMessage(metadata);

            if (m.getData().equals("addHashTagToChannel")) {
                Message m2 = super.receiveMessage(metadata);
                Message m3 = super.receiveMessage(metadata);

                String channel = m2.getData();
                String tag = m3.getData();

                System.out.println("addHashTagToChannel: " + channel + " " + tag);

                newdata.getCatalog().addHashTagToChannel(channel, tag);
            }

            if (m.getData().equals("addHashTagToAVideoChannel")) {
                Message m2 = super.receiveMessage(metadata);
                Message m3 = super.receiveMessage(metadata);
                Message m4 = super.receiveMessage(metadata);

                String channel = m2.getData();
                String video = m3.getData();
                String tag = m4.getData();

                System.out.println("addHashTagToAVideoChannel: " + channel + " " + video + " " + tag);

                newdata.getCatalog().addHashTagToAVideoChannel(channel, video, tag);
            }

            if (m.getData().equals("removeHashTagFromChannel")) {
                Message m2 = super.receiveMessage(metadata);
                Message m3 = super.receiveMessage(metadata);
                ...
                System.out.println("removeHashTagFromChannel: " + channel + " " + tag);


                newdata.getCatalog().removeHashTagFromChannel(channel, tag);
            }
            if (m.getData().equals("removeHashTagFromVideo")) {
                Message m2 = super.receiveMessage(metadata);
                Message m3 = super.receiveMessage(metadata);
                Message m4 = super.receiveMessage(metadata);
                ...
                System.out.println("removeHashTagFromVideo: " + channel + " " + video + " " + tag);

                newdata.getCatalog().removeHashTagFromVideo(channel, video, tag);
            }
            if (m.getData().equals("addVideo")) {
                Message m2 = super.receiveMessage(metadata);
                Message m3 = super.receiveMessage(metadata);
                ...
                System.out.println("addVideo: " + channel + " " + video);

                newdata.getCatalog().addVideo(channel, video);
            }
            if (m.getData().equals("removeVideo")) {
                Message m2 = super.receiveMessage(metadata);
                Message m3 = super.receiveMessage(metadata);
                ...
                System.out.println("removeVideo: " + channel + " " + video);

                newdata.getCatalog().removeVideo(channel, video);
            }
            if (m.getData().equals("createChannel")) {
                Message m2 = super.receiveMessage(metadata);
                String channel = m2.getData();

                System.out.println("createChannel: " + channel);

                newdata.getCatalog().createChannel(channel);
            }
            if (m.getData().equals("deleteChannel")) {
                Message m2 = super.receiveMessage(metadata);
                String channel = m2.getData();

                System.out.println("deleteChannel: " + channel);

                newdata.getCatalog().deleteChannel(channel);
            }
        }
    }
    ...
    public void acceptConnection(ConsumerData consumerData) throws IOException, ClassNotFoundException {
        ...
        while (true) {
            System.out.println("\nWaiting for a command from a consumer ");

            Message m = super.receiveMessage(clientConnectionMetadata);


            if (m.getData().equals("ask channels")) { // 1 Consumer
